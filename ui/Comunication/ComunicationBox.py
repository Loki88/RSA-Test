...
class ComunicationBox(Content):
	...
	def add_status(self, status_widget, next_line, count=True):
		...
		if count:
			current_status.insert(current_status.get_end_iter(), str(self.message_count) + " - " + next_line + "\n")
		else:
			current_status.insert(current_status.get_end_iter(), next_line + "\n")
	...
	def next_round(self):
		line = "*"*32
		self.add_status(self.status, line, False)
